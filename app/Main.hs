...
import MemoryStore
import Encode (encodeBulkString, encodeNullBulkString, encodeSimpleString)
                                                       ...
                                                       where
                                                         ...
                                                         handleArguments v (Just y1 : Just y2 : _) now = case U.bsToLower y1 of
                                                           "px" -> let exp = U.bsToInteger y2
                                                                   in
                                                                     case exp of
                                                                       Nothing -> MemoryStoreEntry v Nothing -- TODO: this would report a command argument error
                                                                       Just y2Int -> MemoryStoreEntry v $ Just (T.ExpireDuration y2Int, T.ExpireReference now)
                                                           "ex" -> let exp = U.bsToInteger y2
                                                                   in
                                                                     case exp of
                                                                       Nothing -> MemoryStoreEntry v Nothing -- TODO: this would report a command argument error
                                                                       Just y2Int -> MemoryStoreEntry v $ Just (T.ExpireDuration $ y2Int * 1000, T.ExpireReference now)
                                                         handleArguments v _ _ = MemoryStoreEntry v Nothing
                                         ...
                                         "get"  -> case xs of
                                                     (Just key : _) -> do
                                                       ...
                                                       case val of
                                                         ...
                                                         Just (MemoryStoreEntry v Nothing) -> send sock $ encodeBulkString v
                                                         Just (MemoryStoreEntry v (Just (T.ExpireDuration exDur, T.ExpireReference exRef))) -> do
